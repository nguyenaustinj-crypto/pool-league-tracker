...
export default async function TeamPage({
  ...
  const access = await requireLeagueView(leagueId, `/leagues/${leagueId}/teams/${teamId}`);
        ...
        <ul className="flex flex-col gap-2">
          ...
          {team.players.length === 0 && (
            <p className="text-neutral-500">
              No players yet.{canEdit && " Add the first one below."}
            </p>
          )}
        </ul>
        {canEdit && (
          ...
          <section className="flex flex-col gap-3 rounded-lg border border-red-300 p-4">
            ...
            {matchCount > 0 ? (
              <p className="text-sm text-neutral-600">
                {team.name} has played in {matchCount} existing match{matchCount === 1 ? "" : "es"}{" "}
                and can&apos;t be deleted. Delete those matches first.
              </p>
            ) : (
              <>
                <p className="text-sm text-neutral-600">
                  This permanently removes {team.name} and every player on it. There&apos;s no
                  undo.
                </p>
