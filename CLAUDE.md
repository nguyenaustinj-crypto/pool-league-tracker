...
- No user accounts. Viewing is public; editing is locked behind one shared editor passcode (see "Editor passcode" below).
...
## Editor passcode (`src/lib/editor.ts`, `src/lib/editor-token.ts`)
Anyone can view everything. Adding or changing anything requires the one shared passcode in the `EDITOR_PASSCODE` env var (Vercel → Settings → Environment Variables, plus local `.env`), entered once per device at `/login`. This is deliberately not user accounts: the goal (2026-09-11) was to lock editing without storing anyone's passwords or personal info.
