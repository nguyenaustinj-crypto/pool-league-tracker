...
League → Player (a flat roster, no persistent Team entity), and Match (belongs to a League) → Round (xN) → Pairing (one home player vs one away player, 2 games/racks each). A match's two sides are picked fresh each time from the league's player pool at match-creation time via a "how many tables?" step — N players per side (N = table count, chosen per match, not fixed), validated server-side to belong to the same league and not overlap. Table *i*'s home/away picks directly become Round 1's pairing *i*; `awayIndexForRound` in `scoring.ts` generalizes the round-robin rotation to any N (originally hardcoded to 3, matching the paper sheet's fixed 3-a-side format). Each side optionally has a free-text `homeLabel`/`awayLabel` (mirrors the paper sheet's blank team-name field); when absent, the UI falls back to listing that side's player names (`src/lib/format.ts`).

Cascade note: `Pairing.homePlayer`/`awayPlayer` cascade-delete, so deleting a League (which cascades to its Players) doesn't hit a foreign-key violation from Pairings still referencing them. The actual protection against deleting an individual player who's mid-match is the app-level check in `deletePlayer` (`src/lib/actions.ts`), not this schema-level cascade.
