...
## Status
V1 built: leagues, a flat player roster (with handicaps) per league, and match entry (pick 3-vs-3 from the league's player pool, with an optional team-name label per side) with auto-computed round-robin pairings and live score calculation, replacing the paper sheet itself. Not yet built: editing/deleting leagues, players, or matches (see `docs/spec-editing.md`); season standings; persistent/named teams.

## Future ideas (not started — get the core app solid first)
- **Editing/delete pass.** Spec'd in `docs/spec-editing.md` — plain CRUD on League/Player/Match, with the one real wrinkle being that editing a match's lineup after scores exist needs to regenerate pairings and warn about erasing those scores.
