...
- Hosting: Vercel, connected to the `nguyenaustinj-crypto/pool-league-tracker` GitHub repo. Database: Prisma Postgres, with a separate free database for local development (see below).
- Sign in with Google (Better Auth, accounts in our own database, no passwords). Viewing is public; changing anything requires a signed-in admin, or, until it's retired, the shared editor passcode. See "Sign-in and editing" below.
...
**Separate dev database (since 2026-09-11):** local `.env` points at its own free Prisma Postgres database (`pool-league-dev` in Vercel Storage, not connected to the Vercel project). The production database's URL lives only in Vercel's environment variables, so nothing run locally can touch real league data. Keep local settings in `.env`, not `.env.local`: Next reads both, but Prisma CLI commands only read `.env`. On a fresh dev database, run `npx prisma migrate deploy` then `npx prisma db seed` (`prisma/seed.ts`, run via `tsx`), which creates "Dev Test League" and refuses to run if the database already has any leagues. Before this, local dev and production shared one database, so older notes about cleaning up test data came from that setup.
...
- **Handicap bonus (stated directly by the league, 2026-09-10 — this is authoritative):** sum each side's player handicaps; anything over 22 is that side's over-22 amount (0 if at or under 22); take the difference between the two sides' over-22 amounts, multiply by 2, and add it, rounded to the nearest whole number, to the side with the *lower* over-22 amount. If neither side is over 22, there's no bonus. Matches the real played sheet exactly (23.4 vs 22.3 → 1.4 and 0.3 over → 1.1 × 2 → +2 to the away side, which the sheet recorded).
- **History:** before this rule was stated, the app used a reading of the league's written rules (Rev 1.6 in `docs/league-rules/`) — base = raw handicap gap × 2, plus a separate over-22 bonus — which gave +3 on that same real sheet instead of the recorded +2. The written rules' worked example (23.2 vs 24.7 → 5) no longer applies; under the stated rule those totals give 3.
...
## Tests & CI
`npm test` (Vitest) covers the two pure-logic modules that decide real league results: `src/lib/scoring.test.ts` and `src/lib/standings.test.ts`. It also covers `src/lib/editor-token.test.ts`: the editor passcode rules (fail closed when unset, 30-day expiry, changing the passcode revokes sessions, tamper checks, no off-site sign-in redirects), and `src/lib/admins.test.ts`: who counts as an admin (listed, Google-verified emails only, case-insensitive, nobody when `ADMIN_EMAILS` is unset). These pin real numbers rather than restating the implementation — every figure the paper sheet recorded for its played round (53 / 38 + 2 = 40), the league-stated handicap rule's edge cases (neither side over 22, exactly 22, one side over, equal amounts over, rounding), the round-robin rotation property for any table count, and the round-wins/half-point-tie standings rules. **If you change the scoring or standings math, expect these to fail: update the expectation and the formula together, deliberately.**
...
Per rule #12, the league's champion is whoever has the most **round wins** across the season, not match wins -- so standings tally every round of every match independently (a match with N tables has N independent round outcomes), with a tied round counting as half a win and half a loss for both teams (footnote #2). Points = the sum of each round's total (score subtotal + handicap bonus) across the season. **Rounds with no scores entered yet (every game 0) are skipped** (`isRoundPlayed` in `scoring.ts`): a played game always has a winner with 10, so all zeros means unplayed. Before 2026-09-11 they counted, which turned every unplayed round into a tie, or into a win for the side the handicap bonus favored. The match list's totals and the score sheet's match total skip them the same way. Computed live from a league's Matches on the league page -- there's no separate "season" entity yet, so this is all-time across everything a League has ever recorded, not scoped to a season boundary.

## Sign-in and editing (`src/lib/auth.ts`, `src/lib/session.ts`, `src/lib/admins.ts`, `src/lib/editor.ts`)
Anyone can view everything. Changing anything requires being an **editor**: `isEditor()` in `src/lib/editor.ts` is true for a signed-in admin, or for a device that entered the shared editor passcode.

**Google sign-in (since 2026-09-18).** Better Auth handles "Sign in with Google" (`src/lib/auth.ts`, endpoints at `/api/auth/*`). Its four tables (`user`, `session`, `account`, `verification`) live in our own database, and there are no passwords. Settings: `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` (Google Cloud project "pool-league-tracker" → Google Auth Platform → Clients; its redirect URIs must include each environment's `/api/auth/callback/google`), `BETTER_AUTH_SECRET` (different per environment), and `BETTER_AUTH_URL`. If the secret or either Google value is missing, `signInConfigured()` is false: sign-in switches off and the site keeps working, instead of every page failing. Sessions last 60 days and extend with use.
- **Admins** are whoever's Google-verified email is listed in the `ADMIN_EMAILS` env var (`src/lib/admins.ts`). That's the only way to grant it, never through the app. Other signed-in users can view but not change anything, until per-league roles land (see `docs/spec-accounts.md`).
- The Google app is in **Testing** mode: only its listed test users and the Cloud project's owner can sign in. Publish it to production before inviting players.
- Better Auth's Prisma models were generated with its CLI. Keep them in sync with that output when upgrading `better-auth`.

**Editor passcode (transitional, `src/lib/editor-token.ts`).** The original lock, kept as a fallback until Google sign-in is proven on the live site, then to be removed. It's one shared passcode in the `EDITOR_PASSCODE` env var, entered once per device under "Use the editor passcode instead" on `/login`.
- Signing in sets an httpOnly cookie holding the sign-in time plus an HMAC of it. The HMAC key is derived from the passcode with scrypt. Sessions last 30 days, checked on the server. **Changing the passcode signs every device out**; that's how to revoke access.
- **Fail closed:** if `EDITOR_PASSCODE` is missing or under 8 characters, the site is read-only for everyone, local dev included.
- Every data-changing server action in `src/lib/actions.ts` calls `await requireEditor()` as its first line. **Any new action must too.** Server actions accept direct POST requests, so hiding buttons isn't protection. Edit-only pages call `requireEditorPage(...)`; shared pages use `isEditor()` to hide edit controls.
- A wrong passcode waits 1 second before answering, and there's no lockout. So the passcode should be a long passphrase, not a short PIN.
- Vercel only picks up a new or changed env var on the next deploy. Redeploy after changing it.

## Status
V1 built: leagues, persistent named Teams per league with a player roster (with handicaps) each, match entry that picks two teams and a variable table count (one player per side per table, drawn from the matching team's roster) with auto-computed round-robin pairings and live score calculation, live standings per league, a full edit/delete pass on League/Team/Player/Match per `docs/spec-editing.md`, Postgres/Vercel deployment (live), and Google sign-in where admins can edit, plus the transitional shared editor passcode (see "Sign-in and editing" above). Editing a match's teams/table count/lineup regenerates its pairings and warns before erasing any scores already entered; deleting a player is blocked while they're part of an existing match, and deleting a team is blocked while it's played in an existing match. Not yet built: a season boundary/reset for standings.
...
- **User accounts + per-league permissions.** People log in, and roles control who can edit a given league's settings/rosters/scores vs. just view them. Real complexity here (login, sessions, password/OAuth) — hold off until there's an actual need for multiple people to independently edit the same league. In progress, planned in `docs/spec-accounts.md`: stage 1 (Google sign-in, site-wide admins via `ADMIN_EMAILS`) is built; next is per-league manager/player roles, joining leagues, and player score entry.
